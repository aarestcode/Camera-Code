/***************************************************************************//**
 * @file	ScienceCamera_ROISetup.cpp
...
int main(int argc, char* argv[]){
    ...
    if( error = ScienceCamera.getROI(offsetX, offsetY, width, height) ) return log.error("Could not read ROI", error);

    // 4. Set ROI
    log.printf("4. Set ROI");
    if( error = ScienceCamera.setROI(atoi(argv[1]), atoi(argv[2]), atoi(argv[3]), atoi(argv[4]))) return log.error("Could not set ROI", error);
